...
dta <- read_csv("Data/200509COVID19MEXICO.csv")
...
```{r eval=FALSE}
  ...
  tbl.restmp <- ans.tmp %>% gather(Tipo, Cantidad, 
                                        "Nuevos Positivo":"Nuevas Pruebas",
                                        factor_key = T)
  ...
  p2 <- tbl.restmp %>% filter(`Tipo` %in% 
                               c("Total Positivo", "Nuevos Decesos",
                                 "Total Pendientes")) %>%
                      ggplot(aes(x = `FECHA_SINTOMAS`, y = `Cantidad`,
                                 grp = `Tipo`, color = `Tipo`)) +
                        geom_line() + 
