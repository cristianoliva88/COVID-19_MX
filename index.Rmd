...
Actualizaré este sitio web diariamente, agregando los datos actualizados de cada día y añadiendo nuevas gráficas y tablas.
...
dta <- read_csv("Data/200617COVID19MEXICO.csv")
...
```{r eval=FALSE}
kk <- tbl.resultado$Nacional %>% #filter(`FECHA_SINTOMAS` <= Sys.Date() - 5 ) %>%
          gather(Tipo, Cantidad, "Nuevos Positivo":"Total Decesos",
                                        factor_key = T)

tbl.kk <- kk %>% filter(`Tipo` %in% c("Nuevos Positivo", "Pendientes") )
    
plot.kk <- tbl.kk %>% ggplot(aes(x = `FECHA_SINTOMAS`, 
                                                 y = `Cantidad`, 
                                                 fill = `Tipo`)) +
...
tbl.time2deathtmp <- dtagrp %>% filter(`RESULTADO` == "Positivo SARS-CoV-2") %>%
                          mutate("Diasparadeceso" = `FECHA_INGRESO` - `FECHA_SINTOMAS`) %>%
                          group_by(`Diasparadeceso`) %>%
                          summarise("Casos" = n()) %>%  ungroup() %>%
                    mutate("Porcentaje" = round(100*`Casos`/sum(`Casos`), 2))
    
plot.kk2<- tbl.time2deathtmp %>% ggplot(aes(x = `Diasparadeceso`,
                                                             y = `Casos`)) +
                  geom_bar(stat = "identity", fill = "darkblue",
                                 aes(text = paste(`Casos`, " fallecieron en ",`Diasparadeceso`, 
                                  " dias ", "(", `Porcentaje`, "%)", sep=""))) +
...
sum(tbl.time2deathtmp$Porcentaje[1:6])


  date.first <- as.Date("2020-01-01")
  date.last <- as.Date("2020-04-28")
  dta.blank <- data.frame("FECHA_SINTOMAS" = seq(date.first, date.last, 1))
  
  dtagrp <- dta_split$Yucatán %>% filter(`FECHA_SINTOMAS` <= date.last &
                                         `FECHA_SINTOMAS` >= date.first)
              

  ans.tmp <- dtagrp %>% group_by(`FECHA_SINTOMAS`) %>%
      summarise("Nuevos Positivo Hosp" = sum(`RESULTADO` == "Positivo SARS-CoV-2" &
                                               `TIPO_PACIENTE` == "HOSPITALIZADO"),
                "Nuevos Positivo Ambu" = sum(`RESULTADO` == "Positivo SARS-CoV-2" &
                                               `TIPO_PACIENTE` == "AMBULATORIO")) %>%
      mutate("Total Positivo Hosp" = cumsum(`Nuevos Positivo Hosp`),
             "Total Positivo Ambu" = cumsum(`Nuevos Positivo Ambu`))
  
  tbl.restmp <- ans.tmp %>% filter(`FECHA_SINTOMAS` > "2020-04-14" ) %>% 
                                gather(Tipo, Cantidad, 
                                        "Nuevos Positivo Hosp":"Total Positivo Ambu",
                                        factor_key = T)
  
  p <- tbl.restmp %>% filter(`Tipo` %in% 
                               c("Nuevos Positivo Hosp", "Nuevos Positivo Ambu")) %>%
                      ggplot(aes(x = `FECHA_SINTOMAS`, y = `Cantidad`,
                                 grp = `Tipo`, color = `Tipo`)) +
                        geom_line() + 
                        geom_point(aes(text = paste(`Cantidad`, 
  ...
  p2 <- tbl.restmp %>% filter(`Tipo` %in% 
                               c(#"Total Positivo Ambu", 
                                 "Total Positivo Hosp")) %>%
                      ggplot(aes(x = `FECHA_SINTOMAS`, y = `Cantidad`,
                                 grp = `Tipo`, color = `Tipo`)) +
                        geom_line() + 
...
xx <- tbl.restmp %>% filter(`Tipo` == "Total Positivo Hosp")
xx <- tbl.restmp %>% filter(`Tipo` == "Total Positivo Ambu")
xx$Cantidad[2:14]/xx$Cantidad[1:13]
  
lm(Cantidad~FECHA_SINTOMAS, data=subset(tbl.restmp, Tipo == "Total Positivo Hosp"))
