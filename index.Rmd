...
description: "Análisis de Casos de COVID-19 en México basado en los Datos Abiertos de la Secretaría de Salud, El COVID-19 es la enfermedad causada por el nuevo coronavirus SARS-CoV-2, Autor: Cristian Oliva Avilés"
url: "http://casoscovidmx.com/"
...
dta <- read_csv("Data/200503COVID19MEXICO.csv")
...
dta.blank.compstate <- expand.grid("FECHA_INGRESO" = dta.blank$FECHA_INGRESO,
...
for(x in grp_names){
  tbl.nuevosdece[[x]] <- dta_comb %>% filter(`grp` == x & !is.na(`FECHA_DEF`) & 
          ...
          group_by(`FECHA_DEF`) %>%
          summarise("grp" = x,
            "Nuevos Decesos" = n()) %>% rename("FECHA_INGRESO" = `FECHA_DEF`)
...
tbl.compstatetmp <- dta_comb %>% group_by(`grp`, `FECHA_INGRESO`) %>%
...
plot.pendcompstate[["Nacional"]] <- tbl.compstatetmp %>%
                      ggplot(aes(x = `FECHA_INGRESO`, y = `Total Pendientes`,
                                 grp = `grp`, color = `grp`)) +
                        geom_line() +
                        geom_point(aes(text = paste(`Total Pendientes`, 
                               'casos pendientes en', `grp`, 'hasta',
                              format(`FECHA_INGRESO`, "%d/%m")))) + 
                        theme_bw() + 
                        theme(legend.title = element_blank()) +
                        scale_color_manual(values = mycolors) + 
...
plot.dececompstate[["Nacional"]] <- tbl.compstatetmp %>%
                      ggplot(aes(x = `FECHA_INGRESO`, y = `Total Decesos`,
                                 grp = `grp`, color = `grp`)) +
                        geom_line() +
...
plot.poscompstatetasa[["Nacional"]] <- tbl.compstatetmp %>%
                      ggplot(aes(x = `FECHA_INGRESO`, y = `Tasa Positivo`,
                                 grp = `grp`, color = `grp`)) +
                        geom_line() +
...
for(x in grp_names){
  ...
  ans.tmp <- left_join(dta.blank, ans.tmp, by="FECHA_INGRESO") %>%
                left_join(tbl.nuevosdece[[x]] %>% 
                            dplyr::select(-c(`grp`)), by = "FECHA_INGRESO")
  ...
  ans.tmp <- ans.tmp %>%  mutate("Total Positivo" = cumsum(`Nuevos Positivo`),
              ...
              "Total Pendientes" = cumsum(`Pendientes`),
              "Total Decesos" = cumsum(`Nuevos Decesos`),
              "Nuevas Pruebas" = `Nuevos Positivo` + `Nuevos No Positivo` +
                                       `Pendientes`)
  ...
  ## Make Plot of Total Positive Cases
    ...
    plot.pruebasnew[[x]] <- tbl.resultado[[x]] %>%
    ...
    plot.tasatotal[[x]] <-  tbl.tasatmp %>%  filter(`Tasa` %in% 
                        ...
                        scale_color_manual(values = c("darkblue","red", 
                                                      "orange")) + 
                        xlab("Fecha de Ingreso/Fecha de Fallecimiento") + 
    ...
    tbl.intubtmp <- dtagrp %>% filter(`RESULTADO` == "Positivo SARS-CoV-2") %>%
                      group_by(`INTUBADO`, `UCI`, .drop = FALSE) %>%
            summarise("Casos" = n()) %>% ungroup() %>%
              mutate("Porcentaje" = round(100*`Casos`/sum(`Casos`), 2))
    ...
    plot.hospcomorb[[x]] <- tbl.hospcomorb %>% ggplot(aes(x = reorder(`COMORB`, -`Porcentaje`), 
                ...
                geom_bar(stat = "identity", fill = "darkblue", 
                  ...
                  theme(legend.text = element_text(size = rel(0.7)),
                        axis.text.x = element_text(
                                        size = rel(0.7), angle = 0)) +
                  xlab("Comorbilidad") + 
                  ylab("% de hospitalizados con cierta comorbilidad") 
                ...
                geom_bar(stat = "identity", fill = "darkblue", 
                  ...
                  theme(legend.text = element_text(size = rel(0.7)),
                        axis.text.x = element_text(
                                        size = rel(0.7), angle = 0)) +
                  xlab("Comorbilidad") + 
                  ylab("% de casos intubados con cierta comorbilidad") 
